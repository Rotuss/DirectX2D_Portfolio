...
void GameEngineShaderResourcesHelper::ResourcesCheck(GameEngineRenderingPipeLine* _Line)
{
	if (nullptr == _Line)
	{
		MsgBoxAssert("파이프라인이 nullptr입니다. 정상적인 파이프라인을 넣어주세요.");
	}
...
void GameEngineShaderResourcesHelper::SetConstantBufferNew(const std::string& _Name, const void* _Data, UINT _Size)
{
	...
	for (; NameStartIter != NameEndIter; ++NameStartIter)
	{
		if (0 == NameStartIter->second.OriginalData.size() || NameStartIter->second.OriginalData.size() != _Size)
		{
...
GameEngineTexture* GameEngineShaderResourcesHelper::SetTexture(const std::string& _Name, const std::string& _TextureName)
{
	...
	{
		MsgBox("쉐이더에서 이러한 이름의 텍스처 세팅를 사용한 적이 없습니다.");
		return nullptr;
	}
...
GameEngineTexture* GameEngineShaderResourcesHelper::SetTexture(const std::string& _Name, GameEngineTexture* _TextureName)
{
	...
	for (; NameStartIter != NameEndIter; ++NameStartIter)
	{
		BindTexture(NameStartIter->second, _TextureName);
	}
...
void GameEngineShaderResourcesHelper::ShaderCheck(GameEngineShader* _Shader)
{
	...
	{
		std::multimap<std::string, GameEngineConstantBufferSetter>::iterator InsertIter = ConstantBufferSettingMap.insert(std::make_pair(Data.first, Data.second));
	...
	{
		std::multimap<std::string, GameEngineTextureSetter>::iterator InsertIter = TextureSettingMap.insert(std::make_pair(Data.first, Data.second));
	...
	{
		std::multimap<std::string, GameEngineSamplerSetter>::iterator InsertIter = SamplerSettingMap.insert(std::make_pair(Data.first, Data.second));
